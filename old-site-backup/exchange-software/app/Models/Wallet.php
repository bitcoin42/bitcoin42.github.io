...
class Wallet extends Model
{
    ...
    public function createAddress($account)
    {
        ...
        $resource = $this->coin->adapter
            ->createAddress($this->resource, $label);
    ...
    {
        return coin(
            $this->coin->adapter->estimateTransactionFee($inputs, $outputs, $amount, $this->min_conf),
            $this->coin
        );
    }
    ...
    {
       return convertCurrency($this->coin->getDollarPrice(), 'USD', $currency, false);
    }
    ...
    {
       return convertCurrency($this->coin->getDollarPrice(), 'USD', $currency, true);
    }
    ...
    {
        $chart = collect($this->coin->getMarketChart($days));

        return $chart->map(function ($data) use ($currency) {
            list($timestamp, $dollarPrice) = $data;

            $from = Money::USD($dollarPrice, true);
            $converted = app('exchanger')->convert($from, new Currency($currency));
