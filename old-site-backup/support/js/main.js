    ...
    */
    ...
    var SBF = {
        ...
        event: function (name, parameters) {
            ...
            let webhooks = admin ? SB_ADMIN_SETTINGS['webhooks'] : CHAT_SETTINGS['webhooks'];
    ...
    */
    ...
    class SBMessage {
        constructor(details = {}) {
            ...
            this.set('payload', payload && !['boolean', 'string'].includes(typeof payload) ? JSON.parse(this.get('payload').replace("\\'", "'")) : {});
        }
        ...
        headerAgent: function () {
            if (!admin && !tickets && !this.dashboard && this.conversation != false && (this.agent_id == -1 || (this.conversation.getLastMessage() != false && SBF.isAgent(this.conversation.getLastMessage().get('user_type')) && this.conversation.getLastMessage().get('user_id') != this.agent_id))) {
                ...
                if (agent) {
                    ...
                    this.headerReset();
                    chat_header.addClass('sb-header-agent').attr('data-agent-id', this.agent_id).html(`<div class="sb-dashboard-btn sb-icon-arrow-left"></div><div class="sb-profile"><img src="${agent['profile_image']}" /><div><span class="sb-name">${agent['full_name']}</span><span class="sb-status">${sb_('Away')}</span></div><i class="sb-icon sb-icon-close sb-responsive-close-btn"></i></div><div class="sb-label-date-top"></div>`);
    ...
    var SBForm = {
        ...
        get: function (input) {
            ...
            switch (type) {
                ...
                case 'select-input':
                    return [SBF.escape(input.find('select,input[disabled]').val() + input.find('input[type="tel"]').val()), name];
                default:
