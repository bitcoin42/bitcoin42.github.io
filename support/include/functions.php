...
$sb_apps = ['dialogflow', 'slack', 'wordpress', 'tickets', 'woocommerce', 'ump', 'perfex', 'whmcs', 'aecommerce', 'messenger', 'whatsapp', 'armember', 'telegram', 'wechat'];
...
function sb_update_user($user_id, $settings, $settings_extra = [], $hash_password = true) {
    ...
    if ($email) {
        ...
        $existing_email = sb_db_get('SELECT user_type, email FROM sb_users WHERE email = "' . $email . '" AND id <> ' . sb_db_escape($user_id));
        if ($existing_email) {
            if (sb_get_setting('duplicate-emails') && !sb_is_agent($existing_email['user_type'])) {
...
function sb_update_conversation_agent($conversation_id, $agent_id, $message = false) {
    ...
    $response = sb_db_query('UPDATE sb_conversations SET agent_id = ' . ($empty_agent_id ? 'NULL' : sb_db_escape($agent_id)) . ', status_code = 2 WHERE id = '. $conversation_id);
    if ($response) {
        if ($message) {
...
function sb_direct_message($user_ids, $message, $subject = false) {
    ...
    $conversation_ids = sb_db_get('SELECT user_id, id FROM sb_conversations WHERE user_id IN (' . $user_ids_string . ') GROUP BY user_id', false);
    ...
    for ($i = 0; $i < $count; $i++) {
        $query .= '(' . $active_user_id  . ', "' . sb_db_escape(sb_merge_fields($message, [sb_get_user($conversation_ids[$i]['user_id'])])) . '", "' . $now . '", 0, "", "", ' . $conversation_ids[$i]['id'] . '),';
    }
...
function sb_send_agents_notifications($recipients, $message, $bottom_message = false, $conversation_id = false, $attachments = false, $user = false, $department = false, $extra = false) {
    ...
    if ($force || sb_get_multi_setting('push-notifications', 'push-notifications-active')) {
        sb_push_notification($user_name, $message, $user['profile_image'], $recipient_id ? $recipient_id : $recipients, $conversation_id);
    }
    if (sb_pusher_active()) {
...
function sb_get_last_message($conversation_id, $exclude_message = false, $user_id = false) {
    return sb_db_get('SELECT message, attachments, payload FROM sb_messages WHERE (message <> "" || attachments <> "")' . ($exclude_message ? (' AND message <> "' . sb_db_escape($exclude_message) . '"') : '') . ' AND conversation_id = ' . sb_db_escape($conversation_id) . ($user_id ? (' AND user_id = ' . sb_db_escape($user_id)) : '') . ' ORDER BY id DESC LIMIT 1');
}
...
function sb_queue($conversation_id, $department = -1) {
    ...
    $conversation = sb_db_get('SELECT user_id, agent_id FROM sb_conversations WHERE id = ' .sb_db_escape($conversation_id));
    ...
    if (count($queue) == 0 || $position == 1) {
        ...
        $cuncurrent_chats = !$cuncurrent_chats || $cuncurrent_chats['queue-concurrent-chats'] == '' ? 5 : intval($cuncurrent_chats['queue-concurrent-chats']);
        $smaller = false;
        $pusher = sb_pusher_active();
        ...
        if ($smaller !== false) {
            ...
            sb_send_message(sb_get_bot_id(), $conversation_id, sb_(sb_get_multi_setting('queue', 'queue-message-success', 'It\'s your turn! An agent will reply to you shortly.')), [], 2);
            sb_send_agents_notifications($smaller['agent_id'], sb_isset(sb_get_last_message($conversation_id, false, $user_id), 'message'), false, $conversation_id);
        } else if ($position == 0) {
            array_push($queue, [$conversation_id, $unix_now, $department]);
            $position = $index + 1;
        }
...
function sb_send_message($sender_id, $conversation_id, $message = '', $attachments = [], $conversation_status_code = -1, $payload = false, $queue = false, $recipient_id = false) {
    ...
    if ($sender_id != -1) {
        ...
        if ($security || $user_id == sb_get_active_user_ID() || !empty($GLOBALS['SB_FORCE_ADMIN'])) {
            ...
            $recipient_id = false;
            $queue_active = sb_get_multi_setting('queue', 'queue-active') && empty($conversation['agent_id']);
            if ($is_sender_agent) {
            ...
            if (!$queue_active && !$is_sender_bot && !$is_human_takeover) {
                ...
                if (($agent_check && (sb_get_setting('notify-agent-email') || sb_get_multi_setting('email-piping', 'email-piping-active'))) || ($user_check && !empty($user['email']) && sb_get_setting('notify-user-email'))) {
    ...
    if ($message) {
        ...
        switch ($conversation_details['source']) {
            ...
            case 'tg':
                ...
                break;
            case 'wc':
                sb_wechat_send_message(sb_get_user_extra($conversation_details['user_id'], 'wechat-id'), $message);
                break;
        }
    }
    return false;
...
function sb_get_admin_language($user_id = false) {
    $language = trim(strtolower(defined('SB_ADMIN_LANG') ? SB_ADMIN_LANG : (sb_get_user_language(sb_get_setting('admin-auto-translations') ? ($user_id ? $user_id : sb_get_active_user_ID()) : false))));
...
function sb_get_block_setting($value) {
    switch ($value)  {
        ...
        case 'popup':
            ...
            return ['active' => sb_isset($settings, 'popup-active'), 'title' => sb_rich_value($settings['popup-title']), 'message' => nl2br(sb_rich_value($settings['popup-msg'])), 'image' => $settings['popup-image']];
        case 'welcome':
...
function sb_push_notification($title = '', $message = '', $icon = '', $interest = false, $conversation_id = false, $user_id = false) {
    ...
    $query = ',"web":{"notification":{"title":"' . str_replace('"', '', $title) . '","body":"' . str_replace('"', '', sb_clear_text_formatting(preg_replace('/\r|\n/', ' ', $message))) . '","icon":"' . $icon . '"' . (empty($link) ? '' : ',"deep_link":"' . $link . '"' ) . ',"hide_notification_if_site_has_focus":true}, "data": {"conversation_id":"' . $conversation_id . '","user_id":"' . ($user_id ? $user_id : sb_get_active_user_ID()) . '"}}}';
...
function sb_curl($url, $post_fields = '', $header = [], $type = 'POST') {
    ...
    switch ($type) {
        ...
        case 'UPLOAD':
            curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 300);
            curl_setopt($ch, CURLOPT_TIMEOUT, 400);
            curl_setopt($ch, CURLOPT_POSTFIELDS, ['media' => new CURLFile($post_fields)]);
...
function sb_download_file($url, $file_name = false, $mime = false) {
                                    ...
                                    if (!empty($shortcode['success'])) {
                                        ...
                                        switch ($source_name) {
                                            ...
                                            case 'tg':
                                                ...
                                                break;
                                            case 'wc':
                                                sb_wechat_send_message($source['open_id'], $message_new);
                                                break;
                                        }
                    ...
                    if (!$valid && !empty($shortcode['required-messaging-apps'])) {
                        ...
                        switch ($source_name) {
                            ...
                            case 'tg':
                                ...
                                break;
                            case 'wc':
                                sb_wechat_send_message($source['open_id'], $last_message);
                                break;
                        }
                ...
                if ($messate_text || isset($messages[$i]['attachments'])) {
                    switch ($source_name) {
                        ...
                        case 'tg':
                            ...
                            break;
                        case 'wc':
                            sb_wechat_send_message($source['open_id'], $last_message, $message_attachments);
                            break;
                    }
                }
                if ($payload) {
                    ...
                    switch ($source_name) {
                        case 'ig':
                        case 'fb':
                            $args = ['source' => $source_name, 'psid' => $source['psid'], 'page_id' => $source['page_id'], 'attachments' => $attachments];
                            break;
                        case 'wa':
                        ...
                        case 'tg':
                            ...
                            break;
                        case 'wc':
                            $args = ['source' => 'wc', 'open_id' => $source['open_id'], 'attachments' => $attachments];
                            break;
                    }
            ...
            if ($bot_message) {
                switch ($source_name) {
                    ...
                    case 'tg':
                        ...
                        break;
                    case 'wc':
                        sb_wechat_send_message($source['open_id'], $bot_message['message']);
                        break;
                }
...
function sb_cloud_membership_validation() {
    ...
    if (!$membership || !isset($membership['count']) || intval($membership['count']) >= intval($membership['quota']) || ($membership['price'] != 0 && time() > $expiration->getTimestamp())) die('<script>document.location = "' . CLOUD_URL . '/account"</script>');
}
