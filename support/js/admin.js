            ...
            if (user_id === false && conversation_id) {
                ...
                if (conversation_id != -1) {
                    activeUser().getFullConversation(conversation_id, (response) => {
                        ...
                        conversations_area.find('.sb-top [data-value="read"]').sbActive(conversation_status_code == 2);
                        conversations_area.find('.sb-conversation-busy').remove();
                        this.updateUserDetails();
                            ...
                            if (!id_check.includes(response[i]['conversation_id'])) {
                                ...
                                if (active_conversation) {
                                    ...
                                    if (conversation) {
                                        ...
                                        conversations_area.find('.sb-top [data-value="read"]').sbActive(status == 2);
                                    } else {
    ...
    var SBProfile = {
        ...
        populate: function (user, profile_area) {
            ...
            if (profile_area.hasClass('sb-profile-list-conversation') && SBChat.conversation) {
                ...
                if (!SBF.null(source)) {
                    code += this.profileRow('conversation-source', source == 'fb' ? 'Facebook' : (source == 'wa' ? 'WhatsApp' : (source == 'tm' ? 'Text message' : (source == 'ig' ? 'Instagram' : (source == 'tg' ? 'Telegram' : (source == 'tk' ? 'Tickets' : (source == 'wc' ? 'WeChat' : source)))))), sb_('Source'));
                }
            }
    ...
    $(document).ready(function () {
        ...
        $(header).on('click', '.sb-version', function () {
            ...
            SBF.ajax({
                ...
                let code = '';
                let names = { 'sb': 'Support Board', 'slack': 'Slack', 'dialogflow': 'Dialogflow', 'tickets': 'Tickets', 'woocommerce': 'Woocommerce', 'ump': 'Ultimate Membership Pro', 'perfex': 'Perfex', 'whmcs': 'WHMCS', 'aecommerce': 'Active eCommerce', 'messenger': 'Messenger', 'whatsapp': 'WhatsApp', 'armember': 'ARMember', 'telegram': 'Telegram', 'wechat': 'WeChat' };
                let updates = false;
                ...
                dialog(message, 'alert', function () {
                    SBF.ajax({
                        ...
                        if (value == 'read') {
                            ...
                            conversations_area.find('.sb-top [data-value="read"]').sbActive(false);
                        } else {
            ...
            if (SBApps.whatsapp.check(SBChat.conversation)) {
                SBApps.whatsapp.send(SBApps.whatsapp.activeUserPhone(), response.message, response.attachments, (response) => {
                    if ('ErrorCode' in response) {
                        dialog(message_part + ' WhatsApp' + ': ' + response.errorMessage, 'info');
                    }
                });
            }
        ...
        */
        ...
        // Slack  
        $(settings_area).on('click', '#slack-test .sb-btn', () => {
        ...
        $(settings_area).on('click', '#whatsapp-twilio-btn .sb-btn, #sms-btn .sb-btn, #wechat-btn .sb-btn', function (e) {
            let id = $(this).closest('[id]').attr('id');
            dialog(`<pre>${SB_URL + (id == 'sms-btn' ? '/inlude/api.php' : (id == 'wechat-btn' ? '/apps/wechat/post.php' : '/apps/whatsapp/post.php')) + cloudURL().replace('&', '?')}</pre>`, 'info');
            return false;
            e.preventDefault();
        });
